...
from joker import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"joker/plugins/{shortname}.py")
        checkplugins(path)
        name = "joker.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"joker/plugins/{shortname}.py")
            name = f"joker.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["joker.plugins." + shortname] = mod
        LOGS.info("᯽︙تم بنجاح تحميل ملف ✓" + shortname)
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"joker/plugins/assistant/{shortname}.py")
        name = "joker.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["joker.plugins.assistant" + shortname] = mod
