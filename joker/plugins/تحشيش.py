...
async def permalink(mention):
    ...
    await edit_or_reply(mention, f"🚻 ** ᯽︙  المستخدم => • ** [{joker}](tg://user?id={user.id}) \n ☑️ **᯽︙  تم رفعها مرتك بواسطه  :**{my_mention} 👰🏼‍♀️.\n**᯽︙  يلا حبيبي امشي نخلف بيبي 👶🏻🤤** ")
...
@l313l.on(admin_cmd(pattern="زين(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    await edit_or_reply(mention, f"زين مطور سورس كرستين @devpokemon")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ الحلو** 「[{joker}](tg://user?id={user.id})」 \n**᯽︙  تـم رفعه مميز بواسطة :** {my_mention}")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"᯽︙ ** لقد تم زواجك/ج من : **[{joker}](tg://user?id={user.id}) 💍\n**᯽︙  الف الف مبروك الان يمكنك اخذ راحتك ** ")
...
@l313l.on(events.NewMessage(outgoing=False, pattern='مين الهقر؟'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownersayed1_id :
        order = await event.reply('انته الهقر يا زين ❤️')
