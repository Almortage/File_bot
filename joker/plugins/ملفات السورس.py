...
@l313l.ar_cmd(
    ...
    info={
        "header": "To list all plugins in joker.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in joker"
    cmd = "ls joker/plugins"
...
@l313l.ar_cmd(
    ...
    info={
        "header": "To list all environment values in joker.",
        "description": "to show all heroku vars/Config values in your joker",
        "usage": "{tr}env",
    },
)
async def _(event):
    "To show all config values in joker"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
        f"**[ĊŖ](tg://need_update_for_some_feature/) قـائمـة الـفـارات:**\n\n\n{o}\n\n**انتبه هنالك معلومات حساسة لا تُعطِها لشخص غير موثوق**"
    )
