...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "سورس كرستين يمر من هنا🎀 قناة السورس : @S_EG_P"
)
...
async def _(event):
    ...
    if replied_user.id == 1355571767:
        return await edit_delete(event, "**لا تحاول تنتحل اصحاب السورس !**")
...
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @S_EG_P")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
