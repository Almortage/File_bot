...
ZelzalVP_cmd = (
    "𓆩 [𝙲.𝚁 𝚂𝙾𝚄𝚁𝙲𝙴 - اوامـر الفـارات](t.me/S_EG_P) 𓆪\n\n"
    ...
    "\n𓆩 [𝙲.𝚁  𝙑𝙖𝙧𝙨 - قنـاة الفـارات](t.me/cristin_so) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر {} بنجـاح ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت كرستين يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@l313l.ar_cmd(pattern="حذف(?:\s|$)([\s\S]*)")
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**✾╎تم حـذف {} بنجـاح ☑️**\n**✾╎المتغيـر المحـذوف : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت كرستين يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@l313l.ar_cmd(pattern="جلب(?:\s|$)([\s\S]*)")
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        if variable is None:
            ...
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎قيمـة الفـار : ↶**\n `{}` \n\n**✾╎قنـاة السـورس : @S_EG_P**".format(input_str, variable))
...
async def variable(event):
    ...
    if input_str == "العراق" or input_str == "عراق":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المتغير : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت كرستين يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@l313l.ar_cmd(pattern="اضف صورة (الحماية|الحمايه|الفحص|الوقتي|البوت|الاوامر) ?(.*)")
async def _(malatha):
    if malatha.fwd_from:
        return
    ...
    if malatha.reply_to_msg_id:
        ...
        if input_str in ["الحماية", "الحمايه"]:
            ...
            try:
                ...
                await zed.edit("**✾╎تم تغييـر صـورة {} .. بنجـاح ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n\n**✾╎قنـاة السـورس : @S_EG_P**".format(input_str, vinfo))
