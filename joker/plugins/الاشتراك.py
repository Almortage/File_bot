...
from joker import BOTLOG_CHATID
...
@l313l.ar_cmd(pattern="(ضع الاشتراك خاص|وضع الاشتراك خاص)(?:\s|$)([\s\S]*)")
async def _(event):
...
@l313l.ar_cmd(pattern="(ضع الاشتراك كروب|وضع الاشتراك كروب)(?:\s|$)([\s\S]*)")
async def _(event):
    if input_str := event.pattern_match.group(2):
        try:
            p = await event.client.get_entity(input_str)
...
async def supc(event):  # Zed-Thon - ZelZal
    ...
    zed_dev = (1001132193, 627658332, 5190136458, 5092041118, 1050898456)
    ...
    if gvarstatus ("sub_private"):
        try:
            ...
            if not ch:
                return await zedub.tgbot.send_message(BOTLOG_CHATID, "**❈╎عـذراً .. قم بوضع قنـاة الاشتـراك اولاً ؟!**")
            try:
                ch = int(ch)
            ...
            if "error_code" in reqt:
                await zedub.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ غير معروف قم باعادة توجيه الرسالة ل@devpokmon لحل المشكلة\n{reqt}**")
...
async def supg(event):  # Zed-Thon - ZelZal
    if event.is_private:
        return
    chat = await event.get_chat()
        ...
        try:
            ...
            if "left" in reqt:
                try:
                    ...
                    await zedub.tgbot.send_message(BOTLOG_CHATID, f"- خطـأ \n{er}")
            if "error_code" in reqt:
                await zedub.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ غير معروف قم باعادة توجيه الرسالة ل@devpokemon لحل المشكلة\n{reqt}**")
