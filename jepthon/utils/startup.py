    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph//file/b489a489ea1b536511c42.jpg",
                caption="**᯽︙ بــوت كرستين يـعـمـل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("سورس الجوكر", "https://t.me/S_EG_P"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await jepiq.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await jepiq.send_message("@BotFather", cr source)
            await asyncio.sleep(1)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "بار الاشعارات", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
